###############incorrect value pe input statement se pehle aur sahii hone par ek line ka gap dena(jab sab hojaye)
import time
import tabulate
...
def again(input_field,count):
     ...
     return input_field
#############
def check_empty(field,typee):#type1 mei sirf empty type2 mei empty, datatype and required ans
     if typee==4:
          counting=1
          cur.execute("select cust_userid from cust_mst")
          storage1=cur.fetchall()
          #while field.strip()=="" or len(field.split())!=1 or (field,) not in storage1:
          ...
          counting=1
          while field.strip()=="" or not ''.join('1.1'.split('.')).isdigit() or float(field)>balance or float(field)<0:
               if counting<2:
          ...
          counting=1
          while field.strip()=="" or not ''.join(field.split('.')).isdigit() or float(field)<=0:
               if counting<2:
                    field=input("                                               You have entered incorrect value, please re-enter >>  ")
               else:
                    field=input("                            You have entered incorrect value, please re-enter or type 'q' to end >>  ")
     ...
     ###############incorrect value pe input statement se pehle aur sahii hone par ek line ka gap dena(jab sab hojaye)
          ...
          counting=0
          while not field.isdigit() or len(str(int(field)))!=10:
               if counting<1:
                    field=input("                                               You have entered incorrect value, please re-enter >>  ")
               else:
                    field=input("                            You have entered incorrect value, please re-enter or type 'q' to end >>  ")
          ...
          counting=1
          names=field.split()
          while field.strip()=="" or not ''.join(field.split()).isalpha():
               if counting<2:
                    field=input("                                               You have entered incorrect value, please re-enter >>  ")
               else:
                    field=input("                            You have entered incorrect value, please re-enter or type 'q' to end >>  ")
          ...
          while True:
               try:
                    ...
                    if field<datetime.datetime.now():
                         break
                    else:
                         if counting<1:
                              field=input("                                               You have entered incorrect value, please re enter >>  ")
     ...
     """
          ...
          counting=0
          while field.strip()=="" or '-' not in field or len(field.split('-'))!=3 or not field.replace('-','1').isdigit() or len(field.split('-')[0])!=4 or len(field.split('-')[1])!=2 or len(field.split('-')[2])!=2 or field.split('-')[0]>=datetime.datetime.now().year or field.split('-')[1]>=datetime.datetime.now().month or field.split('-')[2]>=datetime.datetime.now().day:
               if counting<1:
                    field=input("                                               You have entered incorrect value, please re-enter >>  ")
               else:
                    field=input("                            You have entered incorrect value, please re-enter or type 'q' to end >>  ")
               ...
               counting+=1
          return field,counting
"""     #####input wali date aur current date compare karna
...
def acc_check():
     ...
     while ch!=2:#matlab jab sahi value input ki acc mei
          if acc.upper() in ('YES','Y'):
               ...
               break
               #database se iD aur pin check karke aage transac karna
          else:
               print()
               ...
               dob,count9=check_empty(dob,"date")
               if count9>=1 and str(dob).upper()=="Q":
                    ch=2
                    print('',end="")
                    break
               #database mei enter karte samay mysql ke format mei convert karlena
               ...
               while True:
                    ...
                    if newPIN1==newPIN2:
                         #database mei id, pin aur sab detail save karo
                         loading("                                                                       Loading")
...
def menu():
     ...
     while ch!=2:
          ...
          if cur.fetchall()[0]!=(0,):
               ...
               print("                                                               >>  Exit                      (8)")
               listopt=[str(i) for i in range(1,9)]
               if len(storageacc)>1:
                    listopt.append('9')
               ...
               option=input("                                        To Choose any above option, type the number which is in front of them >>  ")
               count27=0
               while option.upper() not in listopt:
                    count27+=1
                    option=again(option,count27)
                    if count27>=2 and option.upper()=='Q':
                         ch=2
                         print('',end="")
                         break
               if ch==2:
...
##########
##########
##########jiske paas ek se jada account hai usko log in karne ke baad option bhi dena dusre account mei log in karne ka(Swap to another Account)
##########above satement same if a person having one account creates another account toh menu mei phir se jaane par option dikhe log in to another account
##########jab account close karna ho toh agar ek hi account hai toh cust_mst se record hata dena aur cust_acc mei closing date daal dena
##########lekin agar ek se jada ho account toh pucho konsa band karna ***aur aur*** saare transaction mei condition daalo ki date closing null ho
##########aur jo account use kar rhe agar vo band kare toh log out hojaye agar ek account hai nhi toh dusre account pe shift hojaye
##########
##########
##########
               if option=='1':
                    ...
                    cur.execute("select cust_userid,cur_bal,bank_IFSC_code from cust_acc where acc_code='"+tran_to_id+"'")
                    ...
                    #yaha par ek aur execute hoga jismei bank_tran mei hoga(sender aur receiver dono ke)
               elif option=='2':
                    #cur.execute("select * from bank_tran where cust_userid='"+newID+"' order by date_time_tran")
                    ...
                    print("                                                               >>  PIN                       (2)")
                    print("                                                               >>  Full Name                 (3)")
                    print("                                                               >>  Address and City          (4)")
                    print("                                                               >>  Mobile Number             (5)")
                    print("                                                               >>  Email Address             (6)")
                    print()
                    up=input("                                        To Choose any above option, type the number which is in front of them >>  ")
                    up,count19=check_empty(up,"opt6")
                    if count19>=2 and up.upper()=="Q":
                         ch=2
                         print('',end="")
                         break
                    print()
                    if up=='1':
                         newID1=input("                                                          Enter your New UserID >>  ")
                         newID1,count20=check_empty(newID1,5)
                         if count20>=2 and newID1.upper()=='Q':
                              ch=2
                              print('',end="")
                              break
                         cur.execute("update cust_mst set cust_userid='"+newID1+"' where cust_userid='"+newID+"'")
                         mydb.commit()
                         cur.execute("update cust_acc set cust_userid='"+newID1+"' where cust_userid='"+newID+"'")
                         mydb.commit()
                         print()
                         loading("                                                                       Loading")
                         print()
                         print("Successfully Updated User ID".center(154))
                         newID=newID1
                    elif up=='2':
                         while True:
                              newPIN1=input("                                                             Enter your New PIN >>  ")
                    ...
                    cur.execute("select acc_code,cur_bal,open_dt,close_dt from cust_acc where cust_userid='"+newID+"'")
                    print(tabulate.tabulate(cur.fetchall(),headers=lasd1,tablefmt="grid"))
               elif option=='6':
                    print()
               ...
               if (count16>=2 and ask.upper()=='Q') or ask.upper() in ('NO','N'):
                    ...
                    print('',end="")
                    break
mydb=mysql.connector.connect(host="localhost",user="root",passwd="iamrohan",database="my_banking_system")
cur=mydb.cursor()
